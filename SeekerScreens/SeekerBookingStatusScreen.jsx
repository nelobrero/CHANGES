...
function SeekerBookingStatusScreen({ navigation, route }) {
  ...
  const handleStarRating = (rating) => {
    ...
    const handleSubmit = async () => {
      ...
      await axios.post("http://172.16.15.247:5001/report/createReport", reportData);
      ...
      await axios.post("http://172.16.15.247:5001/notifications/create", notification)
      setModalVisible(false);
      navigation.goBack();
  };
  const handleReview = () => {
...
const handleSubmitReview = async () => {
  ...
  await axios.post("http://172.16.15.247:5001/rating/createRating", { bookingId: data.bookingId, rating: starCount, comment: reviewText, serviceId: data.serviceId, seekerId: data.seekerId, providerId: data.providerId, images: reviewData.images, seekerImage: data.seekerImage, seekerName: data.seekerName });
  ...
  useEffect(() => {
    const checkForReport = async () => {
      const response = await axios.post("http://172.16.15.247:5001/report/getReportByBookingId", { bookingId: data.bookingId, reporterId: data.seekerId });
      const response2 = await axios.post("http://172.16.15.247:5001/rating/getRatingByBookingId", { bookingId: data.bookingId });
